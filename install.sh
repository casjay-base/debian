...
if [ "$update" == "yes" ]; then
  ...
  git clone -q https://github.com/casjay-base/debian /tmp/debian
  find /tmp/debian -type f -exec sed -i "s#MYHOSTIP#$CURRIP4#g" {} \; >/dev/null 2>&1
  find /tmp/debian -type f -exec sed -i "s#myserverdomainname#$(hostname -f 2>/dev/null)#g" {} \; >/dev/null 2>&1
  sudo rm -Rf /tmp/debian/etc/{apache2,nginx,postfix,samba} >/dev/null 2>&1
  sudo cp -Rf /tmp/debian/{usr,etc,var}* / >/dev/null 2>&1
  sudo rm -Rf /etc/cron.*/0* >/dev/null 2>&1
  sudo rm -Rf /tmp/debian >/dev/null 2>&1
...
###############################################################################################
  ...
  temp_cnt=${wait_time}
  printf "${GREEN}            *** ${RED}•${GREEN} Welcome to my debian Installer ${RED}•${GREEN} ***${NC}\n"
  ...
  sudo rm -Rf /tmp/debian >/dev/null 2>&1
  git clone -q https://github.com/casjay-base/debian /tmp/debian >/dev/null 2>&1

  # Copy apt sources
  printf "\n  ${GREEN}*** ${RED}•${BLUE} copy apt sources ${RED}•${GREEN} ***${NC}\n"
  sudo cp -Rf /tmp/debian/etc/apt/* /etc/apt/ >/dev/null 2>&1
  ...
  CURRIP4="$(/sbin/ifconfig | grep -E "venet|inet" | grep -v "127.0.0." | grep 'inet' | grep -v inet6 | awk '{print $2}' | sed s#addr:##g | head -n1)"
  find /tmp/debian -type f -exec sed -i "s#MYHOSTIP#$CURRIP4#g" {} \; >/dev/null 2>&1
  find /tmp/debian -type f -exec sed -i "s#MYHOSTNAME#$(hostname -s)#g" {} \; >/dev/null 2>&1

  # Copy configurations to system
  printf "\n  ${GREEN}*** ${RED}•${BLUE} copying system files ${RED}•${GREEN} ***${NC}\n"
  chmod -Rf 755 /tmp/debian/usr/local/bin/*.sh >/dev/null 2>&1
  sudo cp -Rf /tmp/debian/{usr,etc,var}* / >/dev/null 2>&1
  mkdir -p /etc/casjaysdev/updates/versions >/dev/null 2>&1
  cp -Rf /tmp/debian/version.txt /etc/casjaysdev/updates/versions/debian.txt >/dev/null 2>&1

  # Cleanup
  rm -Rf /tmp/debian >/dev/null 2>&1
  ...
  #use  sudo GH=username bash -c "$(wget -qO - https://github.com/casjay-base/debian/raw/main/install.sh)"
  if [ -n "$GH" ]; then
  ...
  NEWVERSION="$(curl -Lsq https://github.com/casjay-base/debian/raw/main/version.txt | grep -v "#" | tail -n 1)"
  RESULT=$?
  #if [ $RESULT -eq 0 ]; then
  printf "${GREEN}      *** 😃 installation of debian complete 😃 *** ${NC}\n"
